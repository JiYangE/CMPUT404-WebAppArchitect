...
# From my peer, https://github.com/tywtyw2002
URL_RULE = "^http://(?P<HOST>[A-Za-z0-9\-\.]+)(?P<PORT>:[0-9]+)?(?P<PATH>.*)$"
URL_MATCH_RE = re.compile(URL_RULE, re.I)

# Create a simple Response object with default status code 200
...
# Create a simple Request object handles "GET" and "POST" requests
class HTTPRequest(object):

    # Constructor method, for headers reference: https://book.douban.com/subject/25863515/ page 41
    ...
    def _build_header(self):
        header = "%s %s HTTP/1.1\r\n" % (self.method, self.path)\
               + "Host: %s:%d \r\n" % (self.host, self.port)\
               + "User-Agent: CMPUT404 Assignment 2 -> Web Client \r\n"\
               + "Accept: text/plain\r\n"
    ...
    def parse_url(self):
        ...
        # https://github.com/tywtyw2002/CMPUT404-assignment-web-client
...
class HTTPClient(object):
    ...
    def connect(self):
        ...
        try:
            ...
            sock.connect((self._request_obj.host, self._request_obj.port))   # self.host, self.port
    ...
    # define a request function to handle all kinds of request
...
# Original name 'help' shadows the built-in function, I changed its name
def helpInfo():
